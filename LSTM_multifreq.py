...
def accuracy(spiking_net,decoder,parameters_enc,parameters,test_dataloader,writer,epoch,args):
    ...
    with torch.no_grad():
        for b_idx, (xlocal, ylocal) in enumerate(test_dataloader):
            ...
            c0 = torch.zeros(2,xlocal.shape[1],50).to(parameters['device'])
            h0 = torch.zeros(2,xlocal.shape[1],50).to(parameters['device'])

            # eee,inv = torch.unique(ylocal,return_inverse=True)
            # inv_sorted = torch.argsort(inv)
            # print(eee)
            # print(inv)
            # aaa = torch.where(xlocal[:,inv_sorted,0]==1)
            #
            # plt.scatter(aaa[0],aaa[1],s=0.1)
            # plt.show()
            LSTM_out, (cn,hn) = LSTM(xlocal[:,:,0:1],(c0,h0))
            lif_out = decoder(hn[-1])
            #print(spk_sum.shape)
...
if __name__ == "__main__":
    ...
    if args.nni_opt:
        ...
        log_dir = None
        comment = 'LSTM_multifreq'
    writer = SummaryWriter(log_dir=log_dir, comment=comment)
    criterion_lif = nn.CrossEntropyLoss()
    LSTM = nn.LSTM(input_size=1,hidden_size=50,num_layers=2,batch_first=False).to(parameters['device'])

    optimizing_params = [{'params':LSTM.parameters(),'lr':args.lr_spikes},{'params': decoder.parameters(),'lr':args.lr_dec}]
    ...
    for epoch in epochs:
        ...
        acc = accuracy(LSTM,decoder,parameters_enc,parameters,test_dataloader,writer,epoch,args)
    ...
    torch.save(torch.tensor(acc_coll),os.path.join(os.getcwd(),'seed_coll',f'LSTM_multifreq_acc_coll_{args.seed}.pt'))
    torch.save(decoder.state_dict(),os.path.join(os.getcwd(),'seed_coll',f'LSTM_multifreq_decoder_{args.seed}.pt'))
